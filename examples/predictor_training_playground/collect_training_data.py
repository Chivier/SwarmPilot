...
Training Configuration:
    ...
    1. Default config (get_default_training_config())
    2. Shared config ("training_config" section)
    3. Service-specific config ("service_training_configs" section)

    Example config file with service-specific settings:
    {
        ...
        "service_training_configs": {
            // Service-specific overrides (keys match service names in config)
            "llm_service": {
                "epochs": 300,
                "data_augmentation": {
                    "cv": 0.3,
...
def get_service_training_config(
    config: Dict[str, Any],
    service_name: str
) -> Dict[str, Any]:
    """
    Get training configuration for a specific service.

    Configuration is built by merging in order (later overrides earlier):
    1. Default config from get_default_training_config()
    2. Shared config from config["training_config"]
    3. Service-specific config from config["service_training_configs"][service_name]

    Args:
        config: Full configuration dictionary from config file
        service_name: Service name as defined in config (e.g., "llm_service", "t2vid_service")

    Returns:
        Merged training configuration for the specific service
    """
    # Start with defaults
    training_config = get_default_training_config()

    # Merge shared training config
    if config.get('training_config'):
        training_config = deep_merge_configs(training_config, config['training_config'])

    # Merge service-specific config
    service_configs = config.get('service_training_configs', {})
    if service_name in service_configs:
        training_config = deep_merge_configs(training_config, service_configs[service_name])
    ...
    if 'predictor' in config:
        ...
        try:
            ...
            if llm_samples:
                ...
                llm_model_id = "llm_service_small_model"
                llm_training_config = get_service_training_config(config, llm_service_name)
                llm_quantiles = llm_training_config.get('quantiles', [0.5, 0.9, 0.95, 0.99])

                llm_platform = get_platform_info(config.get(llm_service_name))
                logger.info(f"Training LLM model with {len(llm_samples)} samples, platform: {llm_platform}")
                logger.info(f"LLM training config: {json.dumps(llm_training_config, indent=2)}")

                for p_type in prediction_types:
                    logger.info(f"  Training {p_type} model...")
                    result = await predictor_client.submit_training_data(
                        llm_model_id, llm_platform, p_type, llm_samples, llm_training_config
                    )
                    logger.info(f"  Training result: {result.get('status', 'unknown')}")
                    await validate_model(
                        predictor_client, llm_model_id, llm_platform, p_type, llm_samples,
                        quantiles=llm_quantiles
                    )

            # Train T2Vid Model
            if t2vid_samples:
                t2vid_service_name = "t2vid_service"
                t2vid_model_id = "t2vid"
                t2vid_training_config = get_service_training_config(config, t2vid_service_name)
                t2vid_quantiles = t2vid_training_config.get('quantiles', [0.5, 0.9, 0.95, 0.99])

                t2vid_platform = get_platform_info(config.get(t2vid_service_name))
                logger.info(f"Training T2Vid model with {len(t2vid_samples)} samples, platform: {t2vid_platform}")
                logger.info(f"T2Vid training config: {json.dumps(t2vid_training_config, indent=2)}")

                for p_type in prediction_types:
                    logger.info(f"  Training {p_type} model...")
                    result = await predictor_client.submit_training_data(
                        t2vid_model_id, t2vid_platform, p_type, t2vid_samples, t2vid_training_config
                    )
                    logger.info(f"  Training result: {result.get('status', 'unknown')}")
                    await validate_model(
                        predictor_client, t2vid_model_id, t2vid_platform, p_type, t2vid_samples,
                        quantiles=t2vid_quantiles
                    )
