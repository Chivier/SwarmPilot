...
def prepare(
    dataset: Literal["mcp-atlas", "dataclaw"],
    output_path: str,
    limit: int | None = None,
) -> None:
    """Load a HuggingFace dataset, reshape into ReplayGroups, and write to JSON.

    Args:
        dataset: Which dataset to load ("mcp-atlas" or "dataclaw").
    ...
    elif dataset == "dataclaw":
        from replay.reshapers.dataclaw import DataclawReshaper

        groups = DataclawReshaper().load_and_reshape(limit=limit)
    elif dataset == "swebench-pro":
        from replay.reshapers.swebench_pro import SwebenchProReshaper

        groups = SwebenchProReshaper().load_and_reshape(limit=limit)
    else:
