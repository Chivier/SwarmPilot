...
class TestSubscriptionManagement:
    ...
    async def test_get_subscribed_tasks_nonexistent_connection(
        self, websocket_manager, mock_websocket
    ):
...
class TestBroadcasting:
    ...
    async def test_broadcast_to_multiple_subscribers(self, websocket_manager):
        ...
        await websocket_manager.broadcast_task_result(
            task_id="task-1", status=TaskStatus.COMPLETED, timestamps=timestamps
        )
    ...
    @pytest.mark.asyncio
    async def test_broadcast_removes_subscriptions(
        self, websocket_manager, mock_websocket
    ):
