...
class TestModelKeyGeneration:
    ...
    def test_generate_key_default_prediction_type(self, storage):
        ...
        key = storage.generate_model_key(
            model_id="test-model", platform_info=platform_info
        )
...
class TestModelSaveAndLoad:
    ...
    def test_save_multiple_types_same_model(self, storage):
        ...
        key1 = storage.generate_model_key(
            "model1", platform_info, "expect_error"
        )
        storage.save_model(
            key1,
            {"weights": [1, 2, 3]},
...
class TestListModels:
    ...
    def test_list_models_with_prediction_types(self, storage):
        ...
        expect_error_model = next(
            m for m in models if m["prediction_type"] == "expect_error"
        )
        quantile_model = next(
            m for m in models if m["prediction_type"] == "quantile"
        )
...
class TestDeleteModel:
    ...
    def test_delete_one_type_keeps_other(self, storage):
        ...
        storage.save_model(
            key1, {"weights": [1, 2, 3]}, {"prediction_type": "expect_error"}
        )
        storage.save_model(
            key2, {"weights": [4, 5, 6]}, {"prediction_type": "quantile"}
        )
...
class TestStorageErrorHandling:
    ...
    def test_list_models_with_corrupted_file(self, temp_storage_dir):
        ...
        corrupted_path = os.path.join(
            temp_storage_dir, "corrupted__pytorch__cpu__quantile.joblib"
        )
    ...
    def test_load_model_with_corrupted_file(self, temp_storage_dir):
        ...
        with pytest.raises(
            (KeyError, OSError, ValueError, EOFError, RuntimeError)
        ):
