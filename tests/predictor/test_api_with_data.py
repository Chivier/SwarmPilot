...
DEFAULT_HIDDEN_LAYERS = [
    ...
    32,
    32,
]  # Hidden layer sizes for MLP (or None for default)
...
def calculate_pinball_loss(
    y_true: float, y_pred: float, quantile: float
) -> float:
...
def load_and_split_data(
    ...
    # Shuffle and split
    df_shuffled = df_clean.sample(frac=1, random_state=random_seed).reset_index(
        drop=True
    )
    ...
    print(
        f"Feature stats - Train mean: {train_df[FEATURE_COLUMN].mean():.2f}, "
        f"Test mean: {test_df[FEATURE_COLUMN].mean():.2f}"
    )
    print(
        f"Target stats - Train mean: {train_df[TARGET_COLUMN].mean():.2f}, "
...
def get_available_models(data_file: Path, min_samples: int = 100) -> list[str]:
    ...
    print(
        f"Found {len(available_models)} models with >= {min_samples} samples:"
    )
...
class PredictorServiceManager:
    ...
    def __init__(
        self,
        api_module_path: str = "swarmpilot.predictor.api:app",
        host: str = "0.0.0.0",
        port: int = 8000,
    ):
...
def train_model(
    ...
    response = requests.post(
        f"{API_BASE_URL}/train", json=request_data, timeout=300
    )
...
def predict(model_id: str, features: dict[str, Any]) -> dict[str, Any]:
    ...
    response = requests.post(
        f"{API_BASE_URL}/predict", json=request_data, timeout=10
    )
...
def run_single_model_test(
    ...
    train_df, test_df = load_and_split_data(
        DATA_FILE, model_id=model_id, train_ratio=TRAIN_TEST_SPLIT
    )
    ...
    if len(train_df) < min_train_samples:
        print(
            f"⚠ Warning: Insufficient training data for {model_id}: {len(train_df)} < {min_train_samples}"
        )
        return None

    if len(test_df) < min_test_samples:
        print(
            f"⚠ Warning: Insufficient test data for {model_id}: {len(test_df)} < {min_test_samples}"
        )
        return None
    ...
    eval_results = evaluate_predictions(
        test_df, model_id=model_id, quantiles=quantiles, max_samples=test_size
    )
    ...
    print(f"{'=' * 70}")
    print("Median (q=0.5) Prediction:")
    print(f"  MAPE: {median_mape:.2f}%")
    print(
        f"  Bias: {median_bias:+.2f}% {'(over-prediction)' if median_bias > 0 else '(under-prediction)' if median_bias < 0 else '(unbiased)'}"
    )

    print("\nExpectation (mean of quantiles) Prediction:")
    print(f"  MAPE: {expectation_mape:.2f}%")
    print(
        f"  Bias: {expectation_bias:+.2f}% {'(over-prediction)' if expectation_bias > 0 else '(under-prediction)' if expectation_bias < 0 else '(unbiased)'}"
    )
    ...
    if expectation_mape > 200:
        ...
        print("  This is expected for quantile regression.")
    else:
        assert expectation_mape < 200, (
            f"Expectation MAPE too high: {expectation_mape:.2f}%"
        )
...
def test_quantile_prediction_with_real_data(
    ...
    if not DATA_FILE.exists():
        pytest.skip(
            f"Data file not found: {DATA_FILE}. This test requires real data to run."
        )
    ...
    print(
        f"Epochs: {epochs or DEFAULT_EPOCHS}, Learning Rate: {learning_rate or DEFAULT_LEARNING_RATE}"
    )
    print(f"{'=' * 70}\n")
    ...
    with PredictorServiceManager():
        ...
        print(
            f"{'Model':<50} {'Pinball':<12} {'Med.MAPE':<12} {'Exp.MAPE':<12}"
        )
        print(f"{'-' * 50} {'-' * 12} {'-' * 12} {'-' * 12}")
        ...
        for result in results:
            model_name = (
                result["model_id"][:47] + "..."
                if len(result["model_id"]) > 50
                else result["model_id"]
            )
            print(
                f"{model_name:<50} {result['overall_pinball_loss']:<12.2f} {result['median_mape']:<12.2f} {result['expectation_mape']:<12.2f}"
            )
