...
def generate_realistic_training_data(
    n_samples=30, base_runtime=100, noise_factor=0.1
):
    ...
    for _i in range(n_samples):
        ...
        base = batch_size * 0.5 + sequence_length * 0.1 + hidden_size * 0.05
...
class TestCompleteWorkflow:
    ...
    def test_expect_error_complete_workflow(self, client):
        ...
        train_request = {
            ...
            "training_config": {"epochs": 300, "learning_rate": 0.01},
        }
...
class TestExperimentModeIntegration:
    ...
    def test_experiment_mode_without_trained_model(self, client):
        ...
        predict_request = {
            ...
            "prediction_type": "expect_error",
            "features": {"exp_runtime": 250.0, "batch_size": 32},
        }
        ...
        assert result["error_margin_ms"] == pytest.approx(
            75.0
        )  # 30% of 250 (default CV)
    ...
    def test_switch_between_normal_and_experiment_mode(self, client):
        ...
        assert (
            exp_result["expected_runtime_ms"]
            != normal_result["expected_runtime_ms"]
        )
...
class TestErrorRecovery:
    ...
    def test_invalid_training_then_valid_training(self, client):
        ...
        invalid_request = {
            ...
            "prediction_type": "expect_error",
            "features_list": generate_realistic_training_data(
                n_samples=5
            ),  # Too few
        }
        ...
        valid_request["features_list"] = generate_realistic_training_data(
            n_samples=30
        )
...
class TestRealWorldScenarios:
    ...
    def test_model_versioning_workflow(self, client):
        ...
        """
        training_data_v1 = generate_realistic_training_data(
            n_samples=30, base_runtime=100
        )
        training_data_v2 = generate_realistic_training_data(
            n_samples=50, base_runtime=90
        )  # Improved
    ...
    def test_multi_platform_deployment(self, client):
        ...
        for _hw_type, hw_name, speed_multiplier in platforms:
            # Adjust runtime based on platform speed
            platform_data = [
                {
                    **sample,
                    "runtime_ms": sample["runtime_ms"] * speed_multiplier,
                }
        ...
        for _, hw_name, _ in platforms:
            ...
            response = client.post("/predict", json=predict_request)
            assert response.status_code == 200
            predictions[hw_name] = response.json()["result"][
                "expected_runtime_ms"
            ]
