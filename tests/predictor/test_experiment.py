...
class TestMultimodalDistribution:
    ...
    def test_bimodal_distribution(self):
        ...
        assert q50 < 200, (
            f"Median {q50:.1f} should be closer to dominant mode (50)"
        )
        assert q99 > 400, (
            f"q99 {q99:.1f} should reach secondary mode region (~500)"
        )
    ...
    def test_trimodal_distribution(self):
        ...
        assert 500 < q50 < 3000, (
            f"Median {q50:.1f} should be near dominant mode (1000)"
        )
        # q99 should reach the large batch mode
        assert q99 > 10000, (
            f"q99 {q99:.1f} should reach large batch mode (~15000)"
        )
    ...
    def test_multimodal_expect_error(self):
        ...
        assert error_margin > 50, (
            f"Error margin {error_margin:.1f} should be significant"
        )
    ...
    def test_mode_with_skewness(self):
        ...
        assert q99 > 15000, (
            f"q99 {q99:.1f} should have extended tail due to skewed mode"
        )
    ...
    def test_backward_compatible_unimodal(self):
        ...
        result_no_modes = generate_experiment_prediction(
            "quantile", features_no_modes
        )

        # With empty exp_modes - should also use unimodal
        features_empty = {"exp_runtime": 1000, "exp_modes": []}
        result_empty = generate_experiment_prediction(
            "quantile", features_empty
        )
...
class TestDistributionParameters:
    ...
    def test_custom_cv_quantile(self):
        ...
        result_high = generate_experiment_prediction(
            "quantile", features_high_cv
        )

        # Compare spreads
        spread_low = float(result_low["quantiles"]["0.99"]) - float(
            result_low["quantiles"]["0.5"]
        )
        spread_high = float(result_high["quantiles"]["0.99"]) - float(
            result_high["quantiles"]["0.5"]
        )

        assert (
            spread_high > spread_low * 2
        )  # High CV should have much wider spread
    ...
    def test_skewness_creates_long_tail(self):
        ...
        ratio_sym = float(result_sym["quantiles"]["0.99"]) / float(
            result_sym["quantiles"]["0.5"]
        )
        ratio_skew = float(result_skew["quantiles"]["0.99"]) / float(
            result_skew["quantiles"]["0.5"]
        )
    ...
    def test_type1_b_task_distribution(self):
        ...
        ratio = q99 / q50
        assert ratio > 5.0, (
            f"Expected ratio > 5.0 for long-tail, got {ratio:.2f}"
        )
    ...
    def test_type1_a_task_distribution(self):
        ...
        ratio = q99 / q50
        assert ratio < 2.5, (
            f"Expected ratio < 2.5 for symmetric, got {ratio:.2f}"
        )
...
class TestMultimodalEdgeCases:
    ...
    def test_multimodal_without_rng(self):
        ...
        modes = [{"mean": 100, "cv": 0.2, "weight": 1.0}]
    ...
    def test_multimodal_with_zero_weight_mode(self):
        ...
        samples = generate_multimodal_samples(
            modes, num_samples=10, rng=np.random.RandomState(42)
        )
        assert len(samples) == 10
    ...
    def test_multimodal_sample_count_adjustment(self):
        ...
        samples = generate_multimodal_samples(
            modes, num_samples=7, rng=np.random.RandomState(42)
        )
