...
class TestPlannerConfigValidation:
    ...
    def test_instance_timeout_zero_fails(self):
        ...
        with patch.dict(os.environ, {
            ...
            from swarmpilot.planner.config import PlannerConfig
            config = PlannerConfig()
            with pytest.raises(ValueError, match="INSTANCE_TIMEOUT must be positive"):
                config.validate()
    ...
    def test_instance_max_retries_negative_fails(self):
        ...
        with patch.dict(os.environ, {
            ...
            from swarmpilot.planner.config import PlannerConfig
            config = PlannerConfig()
            with pytest.raises(ValueError, match="INSTANCE_MAX_RETRIES must be non-negative"):
                config.validate()
    ...
    def test_instance_retry_delay_negative_fails(self):
        ...
        with patch.dict(os.environ, {
            ...
            from swarmpilot.planner.config import PlannerConfig
            config = PlannerConfig()
            with pytest.raises(ValueError, match="INSTANCE_RETRY_DELAY must be non-negative"):
                config.validate()
    ...
    def test_planner_port_zero_fails(self):
        ...
        with patch.dict(os.environ, {
            ...
            from swarmpilot.planner.config import PlannerConfig
            config = PlannerConfig()
            with pytest.raises(ValueError, match="PLANNER_PORT must be 1-65535"):
                config.validate()
...
class TestPlannerConfigGetSchedulerUrl:
    ...
    def test_get_scheduler_url_with_override(self):
        ...
        with patch.dict(os.environ, {"SCHEDULER_URL": "http://env-scheduler:8100"}, clear=False):
            from swarmpilot.planner.config import PlannerConfig
            config = PlannerConfig()
...
class TestPlannerConfigInit:
    ...
    def test_auto_optimize_enabled_true_values(self):
        ...
        for value in ["true", "TRUE", "True", "1", "yes", "YES"]:
            with patch.dict(os.environ, {"AUTO_OPTIMIZE_ENABLED": value}, clear=False):
                from swarmpilot.planner.config import PlannerConfig
                config = PlannerConfig()
                assert config.auto_optimize_enabled is True, f"Failed for value: {value}"
    ...
    def test_auto_optimize_enabled_false_values(self):
        ...
        for value in ["false", "FALSE", "0", "no", "anything"]:
            with patch.dict(os.environ, {"AUTO_OPTIMIZE_ENABLED": value}, clear=False):
                from swarmpilot.planner.config import PlannerConfig
                config = PlannerConfig()
                assert config.auto_optimize_enabled is False, f"Failed for value: {value}"

    def test_default_values(self):
        """Test default configuration values."""
        env_minimal = {k: v for k, v in os.environ.items()
                       if k not in ["INSTANCE_TIMEOUT", "INSTANCE_MAX_RETRIES",
                                    "INSTANCE_RETRY_DELAY", "PLANNER_PORT",
                                    "AUTO_OPTIMIZE_ENABLED", "AUTO_OPTIMIZE_INTERVAL"]}
