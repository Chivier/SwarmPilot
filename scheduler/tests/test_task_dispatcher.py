...
class TestTaskDispatcherInit:
    ...
    async def test_initialization(
        self, task_registry, instance_registry, websocket_manager
    ):
...
class TestSuccessfulDispatch:
    ...
    async def test_dispatch_updates_status_to_running(
        self, task_dispatcher, instance_registry, task_registry, sample_instance
    ):
...
class TestErrorHandling:
    ...
    async def test_dispatch_instance_not_found(
        self, task_dispatcher, task_registry, websocket_manager
    ):
    ...
    async def test_dispatch_unexpected_error(
        ...
        # The dispatcher catches all exceptions and returns a generic error
        assert (
            "Task dispatch failed" in task.error
            or "Unexpected error" in task.error
        )
...
class TestWebSocketNotification:
    ...
    async def test_notification_only_for_terminal_states(
        self, task_dispatcher, task_registry
    ):
        ...
        mock_websocket = MagicMock()
        mock_websocket.send_json = AsyncMock()
        await task_dispatcher.websocket_manager.subscribe(
            mock_websocket, ["task-1"]
        )
...
class TestHandleTaskResult:
    ...
    async def test_handle_completed_task(
        self, task_dispatcher, sample_instance
    ):
        ...
        stats = await task_dispatcher.instance_registry.get_stats(
    ...
    async def test_handle_completed_with_training_data(
        ...
        await task_dispatcher.instance_registry.register(sample_instance)
        _task = await task_dispatcher.task_registry.create_task(
            task_id="task-1",
            model_id="model-1",
            task_input={},
            metadata={"input_size": 1000},
        ...
        # Handle completion
        await task_dispatcher.handle_task_result(
            task_id="task-1", status="completed", execution_time_ms=150.0
        )
    ...
    @pytest.mark.asyncio
    async def test_handle_failed_task(self, task_dispatcher, sample_instance):
        ...
        # Handle failure
        await task_dispatcher.handle_task_result(
            task_id="task-1", status="failed", error="Task execution failed"
        )
    ...
    async def test_handle_triggers_queue_update(
        ...
        # Handle completion
        await task_dispatcher.handle_task_result(
            task_id="task-1", status="completed", execution_time_ms=120.0
        )

        # Verify queue was updated
        queue = await task_dispatcher.instance_registry.get_queue_info(
            sample_instance.instance_id
        )
...
class TestQueueUpdateOnCompletion:
    ...
    async def test_update_missing_queue_info(
        self, task_dispatcher, instance_registry, sample_instance
    ):
        ...
        # Should not raise error
        await task_dispatcher._update_queue_on_completion(
            instance_id="inst-1", predicted_time_ms=100.0, actual_time_ms=120.0
        )
...
class TestAsyncDispatch:
    ...
    async def test_dispatch_task_async_creates_task(
        self, task_dispatcher, task_registry, instance_registry, sample_instance
    ):
        ...
        with patch.object(
            task_dispatcher, "dispatch_task", new=AsyncMock()
        ) as mock_dispatch:
            # Call async dispatch
...
class TestClose:
    ...
    @pytest.mark.asyncio
    async def test_close_http_client(self, task_dispatcher):
...
class TestTransientErrorRetry:
    ...
    def test_is_transient_connection_error_read_error(self, task_dispatcher):
    ...
    def test_is_transient_connection_error_connect_error(self, task_dispatcher):
    ...
    def test_is_transient_connection_error_timeout(self, task_dispatcher):
...
class TestThroughputTracking:
    ...
    async def test_handle_completed_records_throughput(
        ...
        mock_tracker.record_execution_time.assert_called_once_with(
            instance_endpoint=sample_instance.endpoint, execution_time_ms=150.0
        )
    ...
    async def test_handle_completed_without_execution_time(
        ...
        await task_dispatcher.handle_task_result(
            task_id="task-1", status="completed", result={"output": "result"}
        )
