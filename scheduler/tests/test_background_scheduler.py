...
class TestBasicFunctionality:
    ...
    async def test_successful_scheduling(
        ...
        mock_scheduling_strategy.schedule_task.return_value = sample_schedule_result
        ...
        mock_task_dispatcher.dispatch_task_async.assert_called_once_with("task-1")
...
class TestTaskNotFound:
    ...
    async def test_task_not_found_after_scheduling(
        ...
        """Test handling when task disappears during scheduling (lines 157-158)."""
        # Setup
        await instance_registry.register(sample_instance)
        await task_registry.create_task(
            task_id="task-1",
        ...
        # Mock task_registry.get to return None after scheduling
        ...
        async def mock_get(task_id):
            ...
            call_count += 1
            if call_count == 1:
                # First call (in schedule_task_background) returns the task
                return await original_get(task_id)
            else:
                # Second call (after scheduling) returns None
                return None
...
class TestSchedulingFailure:
    ...
    async def test_scheduling_strategy_fails(
        ...
        mock_scheduling_strategy.schedule_task.side_effect = Exception("Prediction service error")
...
class TestConcurrency:
    ...
    async def test_semaphore_limits_concurrency(
        ...
        assert stats["active_scheduling_tasks"] >= 0  # Some tasks may have completed
