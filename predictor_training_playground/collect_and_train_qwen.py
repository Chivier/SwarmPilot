#!/usr/bin/env python3
"""Collect runtime data for Qwen3-Next-80B-A3B-Instruct and train a quantile predictor.

Phases:
    A. Deploy 2 vLLM instances via SwarmPilot SDK (Planner → PyLet).
    B. Send chat-completion requests through the Scheduler proxy (round-robin).
    C. Save collected runtime data to a JSON file.
    D. Train a QuantilePredictor MLP (no preprocessor) and save the model.

Usage:
    # Collect 200 samples and train
    uv run python collect_and_train_qwen.py --train

    # Collect only (skip training)
    uv run python collect_and_train_qwen.py --num-requests 100

    # Train from existing data file
    uv run python collect_and_train_qwen.py --load-json qwen_runtime_data.json --train
"""
...
async def deploy_instances(
    ...
    Uses a custom vllm serve command with ``--tensor-parallel-size``
    matching the GPU count so the model is sharded across all GPUs
    ...
    Args:
        ...
        gpu: GPUs per instance (also tensor-parallel-size).
    ...
    logger.info(
        ...
        f"(gpu={gpu}, tensor-parallel-size={gpu})"
    )
...
async def collect_single(
    ...
    try:
        ...
        # Prefer exact prompt_tokens from vLLM; fall back to estimate.
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Collect LLM runtime data and train a quantile predictor."
        ),
    )
    parser.add_argument(
        "--planner-url",
...
async def main() -> None:
    ...
    # Wait for backend to be ready before collecting
    if not args.load_json and not args.skip_deploy:
