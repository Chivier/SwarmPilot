...
for host in "${SLEEP_MODEL_A_HOSTS[@]}"; do
    for instance_port in "${INSTANCE_PORT_LIST[@]}"; do
        (
            instance_id="${host}:${instance_port}"
            response=$(curl -s -X POST "http://$host:$instance_port/model/start" \
                -H "Content-Type: application/json" \
                -d "{\"model_id\": \"llm_service_large_model\", \"scheduler_url\": \"http://$SCHEDULER_A_HOST:$SCHEDULER_PORT\", \"parameters\": {}}")
...
for host in "${SLEEP_MODEL_B_HOSTS[@]}"; do
    for instance_port in "${INSTANCE_PORT_LIST[@]}"; do
        ...
        response=$(curl -s -X POST "http://$host:$instance_port/model/start" \
                -H "Content-Type: application/json" \
                -d "{\"model_id\": \"llm_service_small_model\", \"scheduler_url\": \"http://$SCHEDULER_B_HOST:$SCHEDULER_PORT\", \"parameters\": {}}")
