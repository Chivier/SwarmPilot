#!/usr/bin/env python3
"""
Initial redeploy helper for Exp13 Workflow Benchmark (Simulation Mode).
...
def build_instances(
    ...
    # Group A instances (Model A / LLM) - only even-indexed (registered to Scheduler A)
    for i in range(n1):
    ...
    # Group B instances (Model B / T2Vid) - only even-indexed (registered to Scheduler B)
    for i in range(n2):
        if i % 2 == 0:  # Only even indices are registered to scheduler
            port = port_b_start + i
            endpoint = f"http://{host}:{port}"
...
def main():
    parser = argparse.ArgumentParser(
        description="Redeploy instances via planner /deploy/migration (simulation mode)",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Use defaults (N1=4, N2=2, localhost)
  python type1_redeploy_sim.py

  # Custom instance counts
  python type1_redeploy_sim.py --n1 8 --n2 4

  # Custom host (for remote deployment)
  python type1_redeploy_sim.py --host 192.168.1.100 --n1 10 --n2 6
        """,
    )
    ...
    print("=" * 60)
    print("Redeploy Configuration (Simulation Mode)")
    print("=" * 60)
    ...
    print(f"    Group A:     {actual_n1} of {args.n1} ({args.model_id_a})")
    print(f"    Group B:     {actual_n2} of {args.n2} ({args.model_id_b})")
