...
class ATaskReceiver:
    ...
    async def _submit_b_tasks_for_workflow(self, workflow_id: str):
        """
        Submit the first B task for a given workflow (sequential mode).
        Subsequent B tasks will be submitted by BTaskReceiver when previous ones complete.
        ...
        if not b_tasks:
            self.logger.warning(f"Empty B task list for workflow {workflow_id}")
            return

        self.logger.info(f"Submitting first B task (0/{len(b_tasks)}) for workflow {workflow_id} [SEQUENTIAL MODE]")

        # Submit only the first B task (index 0)
        record = self._submit_b_task(b_tasks[0])
        self.b_submitted.append(record)

        # Update workflow state to track that we've submitted the first B task
        if workflow_id in self.workflow_states:
            self.workflow_states[workflow_id].next_b_task_index = 1

        self.logger.debug(f"Submitted first B task for workflow {workflow_id}, next index: 1")
...
class BTaskReceiver:
    ...
    def __init__(self,
        ...
        Args:
            ...
            scheduler_b_url: HTTP URL for Scheduler B (for task submission)
            b_task_ids: List of all B task IDs to subscribe to
            b_tasks_by_workflow: Map of workflow_id -> list of B task data (for sequential submission)
            workflow_states: Shared workflow state dictionary
        ...
        self.b_submitted_sequential: List[TaskRecord] = []  # Track B tasks submitted sequentially
    ...
    def _submit_next_b_task(self, workflow_id: str) -> bool:
        """
        Submit the next B task in sequence for a workflow.

        Args:
            workflow_id: Workflow ID

        Returns:
            True if a B task was submitted, False if no more B tasks
        """
        ...
        next_index = workflow.next_b_task_index

        # Check if there are more B tasks to submit
        if workflow_id not in self.b_tasks_by_workflow:
            self.logger.error(f"No B tasks found for workflow {workflow_id}")
            return False
        ...
        if next_index >= len(b_tasks):
            # No more B tasks to submit
            self.logger.debug(f"No more B tasks to submit for workflow {workflow_id} "
                            f"(next_index={next_index}, total={len(b_tasks)})")
            return False

        # Submit the next B task
        task_data = b_tasks[next_index]
        ...
        try:
            ...
            if not result.get("success", True):
                error_msg = result.get("error", "Unknown error")
                self.logger.error(f"B task {task_data.task_id} submission failed: {error_msg}")
            ...
            workflow.next_b_task_index = next_index + 1

            self.logger.info(f"[SEQUENTIAL] Submitted B task {next_index}/{len(b_tasks)} "
                           f"for workflow {workflow_id}: {task_data.task_id}")
            return True

        except Exception as e:
            self.logger.error(f"Exception submitting B task {task_data.task_id}: {e}")
    ...
    async def _handle_b_result(self, data: Dict):
        ...
        if status == "completed":
            ...
            # [SEQUENTIAL MODE] Submit the next B task if there is one
            has_next = self._submit_next_b_task(workflow_id)
