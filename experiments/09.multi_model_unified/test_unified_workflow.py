...
This unified experiment integrates experiments 04-07 with mode selection:

Modes:
- ocr (Experiment 04): A → n B tasks (parallel) → Complete
  * Parallel B task execution
  * Workflow completes when all B tasks finish

- t2img (Experiment 05): A → n B tasks (sequential) → Complete
  * Sequential B task execution
  * Next B task submits only after previous completes

- merge (Experiment 06): A → n B tasks (parallel) → Merge A → Complete
  * Parallel B task execution
  * Merge task submitted after all B tasks complete
  * Workflow completes when merge task finishes

- dr (Experiment 07): A → n B1 tasks (parallel) → n B2 tasks (pipelined) → Merge A → Complete
  * B1 and B2 split (slow + fast peaks)
  * Each B1 completion triggers paired B2 submission
  * Pipelined B1→B2 execution
  * Merge task submitted after all B2 tasks complete

Usage:
  python test_unified_workflow.py --mode ocr [options]
  python test_unified_workflow.py --mode t2img [options]
  python test_unified_workflow.py --mode merge [options]
  python test_unified_workflow.py --mode dr [options]
"""
...
from workload_generator import (
    ...
    generate_merge_task_distribution,
...
class WorkflowState:
    """
    Unified state tracking for a single workflow across all modes.

    Fields are conditionally used based on mode:
    - ocr mode: Uses b_task_ids, completed_b_tasks
    - t2img mode: Adds next_b_task_index for sequential submission
    - merge mode: Adds merge_task_id, merge_submit_time, merge_complete_time
    - dr mode: Adds b1_task_ids, b2_task_ids, completed_b1_tasks, completed_b2_tasks
    """
    workflow_id: str
    strategy: str
    a_task_id: str
    ...
    # OCR/T2IMG/Merge modes: B task tracking
    ...
    # T2IMG mode: Sequential submission tracking
    next_b_task_index: int = 0  # Index of next B task to submit

    # Merge/DR modes: Merge task tracking
    merge_task_id: Optional[str] = None
    merge_submit_time: Optional[float] = None
    merge_complete_time: Optional[float] = None

    # DR mode: B1/B2 split tracking
    ...
    def is_complete(self, mode: str = "ocr") -> bool:
        """
        Check if workflow is complete based on mode.

        Args:
            mode: Workflow mode (ocr, t2img, merge, dr)
        """
        if mode in ["ocr", "t2img"]:
            # Workflow complete when all B tasks done
            return self.completed_b_tasks >= self.total_b_tasks
        elif mode == "merge":
            # Workflow complete when merge task done
    ...
    def mark_b_task_complete(self, b_task_id: str, complete_time: float, mode: str = "ocr"):
        ...
        if b_task_id not in self.b_complete_times:
            ...
            # Update workflow complete time if this is the last B task (ocr/t2img modes)
            if mode in ["ocr", "t2img"] and self.is_complete(mode):
    ...
    def mark_b1_task_complete(self, b1_task_id: str, complete_time: float):
        """Mark a B1 task as complete (DR mode)."""
    ...
    def mark_b2_task_complete(self, b2_task_id: str, complete_time: float):
        """Mark a B2 task as complete (DR mode)."""
...
def test_strategy_workflow(
    ...
    # Step 3: Pre-generate all task IDs (including warmup)
    ...
    # Step 4: Generate task data (including warmup tasks)
...
def main(mode: str, num_workflows: int = 100, qps_a: float = 8.0, gqps: Optional[float] = None,
         warmup_ratio: float = 0.0, seed: int = 42, strategies: List[str] = None):
    """
    Main entry point for experiment 09 (unified workflow framework).
    ...
    logger.info(f"Starting Experiment 09 in {mode.upper()} mode")

    # Validate mode and check implementation status
    if mode != "ocr":
        logger.error(f"Mode '{mode}' is not yet implemented in this version.")
        logger.error("Currently only 'ocr' mode is fully implemented.")
        logger.error("To use other modes, please use the original experiments:")
        logger.error("  - t2img mode: Use experiments/05.multi_model_workflow_dynamic_parallel/")
        logger.error("  - merge mode: Use experiments/06.multi_model_workflow_dynamic_merge/")
        logger.error("  - dr mode: Use experiments/07.multi_model_workflow_dynamic_merge_2/")
        return
    ...
    results_file = f"results/results_workflow_{mode}_{timestamp}.json"

    output_data = {
        "experiment": f"09.multi_model_unified_{mode}",
        "mode": mode,
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Experiment 09: Unified Multi-Model Workflow Framework",
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        choices=["ocr", "t2img", "merge", "dr"],
        help="Workflow mode: ocr (parallel B), t2img (sequential B), merge (parallel B + merge), dr (B1/B2 split + merge)"
    )
    ...
    parser.add_argument(
        "--strategies",
        nargs="+",
        default=["round_robin", "min_time", "probabilistic"],
    ...
    # Display mode banner
    mode_names = {
        "ocr": "OCR Mode (Experiment 04) - Parallel B Execution",
        "t2img": "T2IMG Mode (Experiment 05) - Sequential B Execution",
        "merge": "Merge Mode (Experiment 06) - Parallel B + Merge Task",
        "dr": "DR Mode (Experiment 07) - B1/B2 Split + Merge Task"
    }

    print("\n" + "=" * 80)
    print(f"Experiment 09: {mode_names[args.mode]}")
    print("=" * 80)
