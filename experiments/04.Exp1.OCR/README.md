...
This experiment tests workflow dependencies where **each A task generates a variable number (n) of B tasks** that execute in parallel across two independent schedulers. A workflow is complete only when **all B tasks** for that workflow have finished.
...
### Differences from Experiment 03 (1-to-1)

| Aspect | Experiment 03 | Experiment 06 |
|--------|--------------|--------------|
| **Workflow pattern** | 1 A task → 1 B task | 1 A task → n B tasks (n ∈ [3,8]) |
| **B task count** | Fixed (100) | Variable (avg ~550 total) |
| **Workflow completion** | When B task completes | When **all** B tasks complete |
| **Threading model** | 3 threads | 4 threads (separate monitor) |
| **State tracking** | Simple 1-to-1 mapping | Complex workflow state (counter) |
...
| 3 | B Result Receiver | B completion events | Updated workflow state | Track B task completion, detect workflow completion |
