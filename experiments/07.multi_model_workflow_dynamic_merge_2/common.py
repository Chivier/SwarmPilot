#!/usr/bin/env python3
"""
Common utilities for continuous request mode in experiment 07.
...
def force_clear_scheduler_tasks(scheduler_url: str):
    ...
    try:
        response = requests.post(f"{scheduler_url}/task/clear", timeout=10.0)
...
def calculate_makespan(completed_workflows: List) -> Dict:
    ...
    first_submit_time = min(e.a_submit_time for e in target_workflows)
...
def print_continuous_mode_summary(strategy: str, makespan_metrics: Dict,
                                  a_metrics: Dict, b1_metrics: Dict, b2_metrics: Dict,
                                  merge_metrics: Dict, wf_metrics: Dict):
    """
    Print a summary of metrics for continuous request mode (adapted for experiment 07).
    ...
    if a_metrics['avg_completion_time'] > 0:
        ...
        print(f"  P99:        {a_metrics['p99_completion_time']:.2f}s")

    # B1 Model Tasks (Scheduler B)
    print("\nB1 Model Tasks (Scheduler B):")
    ...
    if b1_metrics['avg_completion_time'] > 0:
        ...
        print(f"  P99:        {b1_metrics['p99_completion_time']:.2f}s")

    # B2 Model Tasks (Scheduler B)
    print("\nB2 Model Tasks (Scheduler B):")
    ...
    if b2_metrics['avg_completion_time'] > 0:
        ...
        print(f"  P99:        {b2_metrics['p99_completion_time']:.2f}s")

    # Merge Model Tasks (Scheduler A)
    print("\nMerge Model Tasks (Scheduler A):")
    ...
    if merge_metrics['avg_completion_time'] > 0:
        ...
        print(f"  P99:        {merge_metrics['p99_completion_time']:.2f}s")
