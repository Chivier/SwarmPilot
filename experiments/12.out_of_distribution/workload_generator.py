...
def generate_workflow_with_a1_b1_sampling(
    num_workflows: int,
    seed: int = 42,
    use_a1_for_exp_runtime: bool = False
) -> Tuple[WorkflowWorkload, WorkloadConfig]:
    """
    Generate workflow workload with B1 sleep_time sampled from A1 data.

    This function is designed for out-of-distribution testing where B1 model
    receives sleep_time values from A1 distribution instead of B1 distribution.
    ...
    3. For each workflow:
       - Select one dr_boot value as A1 time
       - Select fanout number of dr_boot values as B1 sleep_time (CHANGED!)
       - Select fanout number of dr_criteria values as B2 times
       - Select one value from dr_summary_dict[fanout] as A2 time
    ...
    Args:
        num_workflows: Number of workflows to generate
        seed: Random seed for reproducibility
        use_a1_for_exp_runtime: If True, set exp_runtime = sleep_time for B1 tasks
                               (for experiment comparison configuration)

    Returns:
        Tuple of (WorkflowWorkload, WorkloadConfig)

    Note:
        The difference from generate_workflow_from_traces is that B1 times
        are sampled from dr_boot (A1 data) instead of dr_query (B1 data).
    """
    random.seed(seed)
    np.random.seed(seed)
    ...
    a1_times = []
    a2_times = []
    b1_times = []  # Will be sampled from A1 (dr_boot)
    b2_times = []
    fanout_values = []
    ...
    for _ in range(num_workflows):
        ...
        # OUT-OF-DISTRIBUTION MODIFICATION:
        # Select fanout number of B1 times from dr_boot (A1 data)
        # instead of dr_query (B1 data)
        b1_workflow = random.sample(dr_boot, fanout)
        b1_times.append(b1_workflow)
    ...
    description_suffix = " (B1 exp_runtime=sleep_time)" if use_a1_for_exp_runtime else ""
    workflow_workload = WorkflowWorkload(
        name="a1_sampled_b1_workflow",
        a1_times=a1_times,
        a2_times=a2_times,
        b1_times=b1_times,
        b2_times=b2_times,
        fanout_values=fanout_values,
        description=f"Workflow with A1-sampled B1: {num_workflows} workflows, "
                    f"A1 from dr_boot, B1 sleep_time from dr_boot (A1), "
                    f"B2 from dr_criteria, A2 from dr_summary_dict" + description_suffix
    )
    ...
    config = WorkloadConfig(
        ...
        mean_time=(all_a1.mean() + all_a2.mean() + all_b1.mean() + all_b2.mean()) / 4,
        std_time=(all_a1.std() + all_a2.std() + all_b1.std() + all_b2.std()) / 4,
        description=f"A1-sampled B1 workflow: {num_workflows} workflows, "
                    f"avg fanout={all_fanouts.mean():.1f}"
    )
