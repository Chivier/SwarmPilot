#!/usr/bin/env python3
"""
Experiment 06: Multi-Model Workflow with Dynamic Fanout and Merge (1-to-n-to-1)

This experiment tests workflow dependencies where each A task generates
a variable number (n) of B tasks that execute in parallel, and upon completion
of all B tasks, a merge A task is triggered. A workflow is complete when the
merge task has finished.
...
- Thread 3: Receive B task results, push to merge_ready_queue when all B tasks done
- Thread 5: Submit merge A tasks (triggered by all B tasks completing)
- Thread 6: Receive merge task results, push final completion events
- Thread 4: Monitor workflow completion (from merge completion), calculate statistics

Key differences from experiment 04 (1-to-n):
- Merge task: After all B tasks complete, submit merge A task (0.5x original A runtime)
- Two-queue architecture: merge_ready_queue (B→merge) and completion_queue (merge→monitor)
- Six-thread design: Added merge task submitter and receiver threads
- Workflow completion: Defined by merge task completion (not B task completion)
"""
...
class WorkflowState:
    ...
    a_task_id: str
    b_task_ids: List[str]
    merge_task_id: str  # NEW: merge A task ID
    total_b_tasks: int
    ...
    all_b_complete_time: Optional[float] = None  # When all B tasks are done
    merge_submit_time: Optional[float] = None  # NEW: when merge task is submitted
    merge_complete_time: Optional[float] = None  # NEW: when merge task completes
    workflow_complete_time: Optional[float] = None  # NEW: same as merge_complete_time

    def are_all_b_tasks_complete(self) -> bool:
        """Check if all B tasks are complete."""
    ...
    def mark_b_task_complete(self, b_task_id: str, complete_time: float):
        ...
        if b_task_id not in self.b_complete_times:
            ...
            # Update all_b_complete_time if this is the last B task
...
class WorkflowCompletionEvent:
    ...
    merge_submit_time: float  # NEW: when merge task was submitted
...
class BTaskReceiver:
    ...
    3. Check if all B tasks are done
    4. If all B tasks done, push to merge_ready_queue for Thread 5 to submit merge task
    """
    ...
    def __init__(self,
        ...
        Args:
            ...
            workflow_states: Shared workflow state dictionary
            merge_ready_queue: Queue to push "all B tasks complete" events
        """
    ...
    async def _handle_b_result(self, data: Dict):
        ...
        if status == "completed":
            ...
            # Check if all B tasks are now complete (ready for merge task)
            if workflow.are_all_b_tasks_complete() and workflow_id not in self.completed_workflows:
                self.completed_workflows.add(workflow_id)
                self._push_merge_ready(workflow)
                self.logger.info(f"Workflow {workflow_id} B tasks all completed "
...
class MergeTaskReceiver:
    ...
    def _push_workflow_completion(self, workflow: WorkflowState):
        ...
        event = WorkflowCompletionEvent(
            ...
            all_b_complete_time=workflow.all_b_complete_time or 0.0,
...
def generate_task_ids(num_workflows: int, fanout_values: List[int], strategy: str) -> tuple[
    ...
    Returns:
        Tuple of (a_task_ids, merge_task_ids, all_b_task_ids, b_task_ids_by_workflow)
    """
    a_task_ids = []
    merge_task_ids = []  # NEW: merge A task IDs
    all_b_task_ids = []
    ...
    return a_task_ids, merge_task_ids, all_b_task_ids, b_task_ids_by_workflow
...
def test_strategy_workflow(
    ...
    a_task_ids, merge_task_ids, all_b_task_ids, b_task_ids_by_workflow = generate_task_ids(
    ...
    logger.info(f"Generated {len(a_task_ids)} A task IDs ({num_warmup_workflows} warmup + {num_workflows} actual), "
               f"{len(merge_task_ids)} merge task IDs, {len(all_b_task_ids)} B task IDs")
    ...
    for i in range(total_workflows):
        ...
        if strategy == 'min_time':
            exp_runtime_a = exp_task_time_a * 1000
            exp_runtime_b = exp_task_time_b * 1000
        else:
        ...
        # NEW: Create merge A task (0.5x execution time of original A)
    ...
    for i in range(total_workflows):
        ...
        workflow_states[workflow_id] = WorkflowState(
            ...
            a_task_id=a_task_ids[i],
            b_task_ids=b_task_ids_by_workflow[workflow_id],
            merge_task_id=merge_task_ids[i],  # NEW: add merge task ID
    ...
    # Step 9: Start Thread 3 (B Task Receiver) - must start before B tasks are submitted
    logger.info("Step 9: Starting Thread 3 (B Task Receiver)")
    b_receiver = BTaskReceiver(
        scheduler_b_ws=SCHEDULER_B_WS,
        b_task_ids=all_b_task_ids,
        workflow_states=workflow_states,
        merge_ready_queue=merge_ready_queue  # NEW: Changed from completion_queue
    )
    b_receiver.start()
    time.sleep(2.0)  # Wait for WebSocket connection

    # Step 10: Start Thread 4 (Workflow Monitor)
    logger.info("Step 10: Starting Thread 4 (Workflow Monitor)")
    ...
    # Step 11: Start Thread 2 (A Task Receiver + B Task Submitter)
    logger.info("Step 11: Starting Thread 2 (A Task Receiver + B Submitter)")
    ...
    a_receiver.stop()
    b_receiver.stop()
    merge_submitter.stop()  # NEW
    merge_receiver.stop()   # NEW
    monitor.stop()
...
def main(num_workflows: int = 100, qps_a: float = 8.0, seed: int = 42,
    ...
    logger.info("Starting Experiment 06: Multi-Model Workflow with Dynamic Fanout and Merge")
    ...
    results_file = f"results/results_workflow_dynamic_merge_{timestamp}.json"

    output_data = {
        "experiment": "06.multi_model_workflow_dynamic_merge",
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Experiment 06: Multi-Model Workflow with Dynamic Fanout and Merge (1-to-n-to-1)",
