...
This experiment tests workflow dependencies where **each A task generates a variable number (n) of B tasks** that execute in parallel, and upon completion of all B tasks, a **merge A task** is triggered. A workflow is complete only when the **merge task** has finished.

### Workflow Pattern

```
A task → n B tasks (parallel) → Merge A task → Workflow Complete
```

### Key Features

- **Dynamic fanout**: Each A task generates 3-8 B tasks (uniform distribution)
- **Parallel B task execution**: All B tasks for a workflow run concurrently
- **Merge task**: After all B tasks complete, a merge A task is submitted (0.5x execution time of original A)
- **Six-thread architecture**: Specialized threads for submission, reception, merge handling, and monitoring
- **Pre-calculated task IDs**: All task IDs (A, B, and merge) generated upfront for WebSocket subscription
...
### Differences from Experiment 04 (1-to-n)

| Aspect | Experiment 04 | Experiment 06 |
|--------|--------------|--------------|
| **Workflow pattern** | 1 A task → n B tasks | 1 A task → n B tasks → 1 merge A task |
| **Merge task** | No | Yes (0.5x original A execution time) |
| **Workflow completion** | When **all** B tasks complete | When **merge A task** completes |
| **Threading model** | 4 threads | 6 threads (+ merge submitter & receiver) |
| **State tracking** | Workflow complete at B completion | Workflow complete at merge completion |
| **Queues** | 1 (completion_queue) | 2 (merge_ready_queue, completion_queue) |
...
Thread 1: A Task Submitter (Poisson)
   ...
   │                 │
   │                 ├──[Extract n]──> Generate n B task submissions
   │                 │
   │                 └──[Submit all]──> Scheduler B ──> Instance Group B (6 instances)
   │                                         │
   │                                         │ (WebSocket completion events for B tasks)
   │                                         ▼
   │                                    Thread 3: B Result Receiver
   │                                         │
   │                                         ├──[Update workflow state]
   │                                         │
   │                                         └──[All B complete]──> merge_ready_queue
   │                                                                     │
   │                                                                     ▼
...
| 2 | A Result Receiver | A completion events | n B task submissions | Receive A results, submit n B tasks per workflow |
| 3 | B Result Receiver | B completion events | merge_ready_queue | Track B task completion, push workflow_id when all B tasks done |
| 5 | Merge Task Submitter | merge_ready_queue | Submitted merge A tasks | Submit merge A task (0.5x runtime) when all B tasks complete |
...
| 4 | Workflow Monitor | completion_queue | Aggregated statistics | Calculate workflow metrics, detect experiment end |
