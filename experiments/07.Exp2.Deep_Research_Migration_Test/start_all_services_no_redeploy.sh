...
for i in $(seq 0 $((N1 - 1))); do
    ...
    (
        response=$(curl -s -X POST "http://localhost:$instance_port/model/start" \
            ...
            -d "{\"model_id\": \"sleep_model_a\", \"scheduler_url\": \"http://localhost:$SCHEDULER_A_PORT\", \"parameters\": {}}")
...
for i in $(seq 0 $((N2 - 1))); do
    ...
    (
        response=$(curl -s -X POST "http://localhost:$instance_port/model/start" \
            -H "Content-Type: application/json" \
            -d "{\"model_id\": \"sleep_model_b\", \"scheduler_url\": \"http://localhost:$SCHEDULER_B_PORT\", \"parameters\": {}}")
