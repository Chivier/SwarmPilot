# Quick Reference: Experiment 07

## One-Line Summary

**1-to-n-to-n pipelined workflow with B1/B2 split**: Each A task generates 3-8 B1 tasks (slow, 7-10s), each B1 triggers one B2 task (fast, 1-3s), workflow completes when merge task finishes.
...
Thread 2 (A Receiver)    ←──WebSocket
        ↓
    Submit n B1 tasks (slow)
        ↓
Thread 2 (B1 Submitter)  →  Scheduler B  →  6 instances
                                ↓
Thread 3 (B1 Receiver)   ←──WebSocket
        ↓
    Submit paired B2 task (fast)
        ↓
Thread 3 (B2 Submitter)  →  Scheduler B (same)
                                ↓
Thread 4 (B2 Receiver)   ←──WebSocket
        ↓
    All B2 complete → merge_ready_queue
        ↓
Thread 5 (Merge Submitter) →  Scheduler A
                                ↓
Thread 6 (Merge Receiver)  ←──WebSocket
        ↓
    completion_queue
        ↓
Thread 7 (Monitor)       ←──Completion queue
```
...
B1 tasks:  task-B1-{strategy}-workflow-{i:04d}-B1-{j:02d}
B2 tasks:  task-B2-{strategy}-workflow-{i:04d}-B2-{j:02d}
Merge:     task-merge-{strategy}-workflow-{i:04d}-merge

Example (workflow 42, fanout=5, strategy=min_time):
  task-A-min_time-workflow-0042-A
  task-B1-min_time-workflow-0042-B1-00
  task-B1-min_time-workflow-0042-B1-01
  ...
  task-B2-min_time-workflow-0042-B2-00  (triggered by B1-00)
  task-B2-min_time-workflow-0042-B2-01  (triggered by B1-01)
  ...
  task-merge-min_time-workflow-0042-merge
```
...
### B1 Tasks (Slow Peak)
- `avg_completion_time`: B1 submit → B1 complete (avg, ~8.5s)
- `p95_completion_time`: B1 submit → B1 complete (P95)

### B2 Tasks (Fast Peak)
- `avg_completion_time`: B2 submit → B2 complete (avg, ~2.0s)
- `p95_completion_time`: B2 submit → B2 complete (P95)

### Merge Tasks
- `avg_completion_time`: Merge submit → Merge complete (avg, 0.5x A time)

### Workflows (KEY)
- `avg_workflow_time`: A submit → merge complete (avg)
- `p95_workflow_time`: A submit → merge complete (P95)
- `avg_fanout`: Average B1/B2 tasks per A task (same count)
...
```
[INFO] Starting Experiment 07
[INFO] Testing strategy: min_time
[INFO] Subscribed to 100 A tasks
[INFO] Subscribed to 547 B1 tasks
[INFO] Subscribed to 547 B2 tasks
[INFO] Subscribed to 100 merge tasks
...
| B1 task avg time (slow) | 7-10s |
| B2 task avg time (fast) | 1-3s |
| Merge task avg time | 2-3.5s (0.5x A) |
| Workflow avg time | 15-20s |
| Workflow P95 | 22-30s |
| Total experiment time | 6-10 minutes |
