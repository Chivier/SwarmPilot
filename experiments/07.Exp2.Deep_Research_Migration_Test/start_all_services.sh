...
# WebSocket port for Scheduler A will be auto-calculated as SCHEDULER_A_PORT + 1
SCHEDULER_B_PORT=8200
# WebSocket port for Scheduler B will be auto-calculated as SCHEDULER_B_PORT + 1
PLANNER_PORT=8202
...
start_service() {
    ...
    # Planner doesn't have a cli, find it from the uvicorn
    if [ -z "$actual_pid" ]; then
        actual_pid=$(pgrep -f "python.*uvicorn src.api:app.*--port $port" | head -1)
    fi
...
# Step 7: Start sleep-model on Group A instances (parallel)
echo ""
...
for i in $(seq 0 $((N1 - 1))); do
    ...
    # half of instance A will be deploy to planner
    # First half uses scheduler A, second half uses planner
    if [ $i -lt $((N1 / 2)) ]; then
        scheduler_url="http://localhost:$SCHEDULER_A_PORT"
    else
        scheduler_url="http://localhost:$PLANNER_PORT"
    fi
    
    echo "Starting model on $instance_id (scheduler: $scheduler_url)..."
    (
        response=$(curl -s -X POST "http://localhost:$instance_port/model/start" \
            -H "Content-Type: application/json" \
            -d "{\"model_id\": \"sleep_model_a\", \"scheduler_url\": \"$scheduler_url\", \"parameters\": {}}")
...
# Step 8: Start sleep-model on Group B instances (parallel)
echo ""
...
for i in $(seq 0 $((N2 - 1))); do
    ...
    if [ $i -lt $((N1 / 2)) ]; then
        scheduler_url="http://localhost:$SCHEDULER_B_PORT"
    else
        scheduler_url="http://localhost:$PLANNER_PORT"
    fi
    ...
    (
        response=$(curl -s -X POST "http://localhost:$instance_port/model/start" \
            -H "Content-Type: application/json" \
            -d "{\"model_id\": \"sleep_model_b\", \"scheduler_url\": \"$scheduler_url\", \"parameters\": {}}")
...
echo "  Scheduler A:  http://localhost:$SCHEDULER_A_PORT (WebSocket: $((SCHEDULER_A_PORT + 1))) - Group A: $N1 instances"
echo "  Scheduler B:  http://localhost:$SCHEDULER_B_PORT (WebSocket: $((SCHEDULER_B_PORT + 1))) - Group B: $N2 instances"
echo ""
