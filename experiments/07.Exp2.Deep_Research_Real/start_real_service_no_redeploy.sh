...
# -----------------------------
# 启动 scheduler / predictor / planner
# -----------------------------

if [[ "$LOCAL_IP" == "$PLANNER_HOST" ]]; then
  # 只有 planner 机器启动 planner（8100）
  start_bg "planner" \
    "cd $PROJECT_ROOT/planner && \
     PLANNER_LOG_DIR=$SCRIPT_DIR/logs/planner \
...
if [[ "$LOCAL_IP" == "$PREDICTOR_HOST" ]]; then
  ...
  start_bg "predictor" \
    "cd $PROJECT_ROOT/predictor && \
     ...
     HTTPS_PROXY=http://hk-mmhttpproxy.woa.com:11113 \
...
if [[ "$LOCAL_IP" == "$SCHEDULER_A_HOST" ]]; then
  ...
  start_bg "scheduler" \
    "cd $PROJECT_ROOT/scheduler && \
     ...
     SCHEDULER_LOG_DIR=$SCRIPT_DIR/logs/scheduler-a \
     SCHEDULER_LOGURU_LEVEL=\"INFO\" \
     PLANNER_URL=http://$PLANNER_HOST:$PLANNER_PORT \
     SCHEDULER_AUTO_REPORT=20.0 \
     PLANNER_REPORT_TIMEOUT=5.0 \
...
if [[ "$LOCAL_IP" == "$SCHEDULER_B_HOST" ]]; then
  ...
  start_bg "scheduler" \
    "cd $PROJECT_ROOT/scheduler && \
     ...
     SCHEDULER_PORT=$SCHEDULER_PORT \
     SCHEDULER_LOG_DIR=$SCRIPT_DIR/logs/scheduler-b \
     SCHEDULER_LOGURU_LEVEL=\"INFO\" \
     PLANNER_URL=http://$PLANNER_HOST:$PLANNER_PORT \
     SCHEDULER_AUTO_REPORT=20.0 \
     PLANNER_REPORT_TIMEOUT=5.0 \
...
cd instance/dockers/llm_service_large_model
uv sync
cd ../llm_service_small_model
uv sync
