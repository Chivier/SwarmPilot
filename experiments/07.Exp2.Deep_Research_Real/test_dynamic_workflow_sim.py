...
class PoissonTaskSubmitter:
    ...
    def _submit_task(self, task_data: WorkflowTaskData) -> TaskRecord:
        ...
        """
        # Build task_input for sleep model simulation
        task_input = {
            "sleep_time": task_data.sleep_time
        }

        # Build metadata with prediction-relevant features
        # For sleep model, only exp_runtime is needed for prediction
        metadata = {
...
def run_strategy_workflow(
    ...
    # Step 4: Prepare sleep time arrays for simulation
    logger.info("Step 4: Preparing sleep time arrays for simulation")
    logger.info(f"Using sleep times - A: {len(task_times_a)} tasks, A2: {len(task_times_a2)} tasks, "
                f"B1: {len(task_times_b1)} tasks, B2: {len(task_times_b2)} tasks")
    ...
    # Track cumulative task counts for indexing into time arrays
    a_task_count = 0
    merge_task_count = 0
    b1_task_count = 0
    b2_task_count = 0
    ...
    for i in range(total_workflows):
        ...
        # Get sleep times for A and merge tasks (cycle if needed)
        if is_warmup and warmup_task_times_a is not None and len(warmup_task_times_a) > 0:
            a_sleep = float(warmup_task_times_a[a_task_count % len(warmup_task_times_a)])
        else:
            a_sleep = float(task_times_a[a_task_count % len(task_times_a)])

        if is_warmup and warmup_task_times_a2 is not None and len(warmup_task_times_a2) > 0:
            merge_sleep = float(warmup_task_times_a2[merge_task_count % len(warmup_task_times_a2)])
        else:
            merge_sleep = float(task_times_a2[merge_task_count % len(task_times_a2)])

        # Create A task (A1) - using sleep model
        exp_runtime_a = a_sleep * 1000.0  # Convert seconds to milliseconds
        a_task = WorkflowTaskData(
            ...
            is_warmup=is_warmup,
            sentence=None,  # No sentence for sleep model
            max_tokens=None  # No max_tokens for sleep model
        )
        a_tasks.append(a_task)
        a_task_count += 1

        # Create merge A task (A2) - using sleep model
        exp_runtime_a2 = merge_sleep * 1000.0  # Convert seconds to milliseconds
        merge_task = WorkflowTaskData(
            task_id=merge_task_id,
            workflow_id=workflow_id,
            task_type="A",  # Submitted to Scheduler A
            sleep_time=merge_sleep,
        ...
        for j in range(n):
            # Get sleep times for B1 and B2 tasks (cycle if needed)
            if is_warmup and warmup_task_times_b1 is not None and len(warmup_task_times_b1) > 0:
                b1_sleep = float(warmup_task_times_b1[b1_task_count % len(warmup_task_times_b1)])
            else:
                b1_sleep = float(task_times_b1[b1_task_count % len(task_times_b1)])

            if is_warmup and warmup_task_times_b2 is not None and len(warmup_task_times_b2) > 0:
                b2_sleep = float(warmup_task_times_b2[b2_task_count % len(warmup_task_times_b2)])
            else:
                b2_sleep = float(task_times_b2[b2_task_count % len(task_times_b2)])

            # Create B1 task - using sleep model
            exp_runtime_b1 = b1_sleep * 1000.0  # Convert seconds to milliseconds
            ...
            # Create B2 task - using sleep model
            exp_runtime_b2 = b2_sleep * 1000.0  # Convert seconds to milliseconds
