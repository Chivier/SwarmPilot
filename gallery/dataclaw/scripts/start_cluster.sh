...
# Usage: bash gallery/dataclaw/scripts/start_cluster.sh [config.yaml]
#
...
check_port() {
    ...
    if lsof -i:"$port" > /dev/null 2>&1; then
        ...
        echo "Run: bash gallery/dataclaw/scripts/stop_cluster.sh"
        return 1
    fi
    return 0
}
...
echo "  Logs:             $LOG_DIR/"
echo ""
echo -e "${YELLOW}Next:${NC} cd gallery/dataclaw && python -m replay.cli run ..."
