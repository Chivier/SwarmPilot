...
bash gallery/replay/benchmark/start_cluster.sh

# 3. Run the benchmark (auto-deploys and tears down models via Planner)
uv run python gallery/replay/benchmark/benchmark_runner.py \
    --data data/mcp-atlas.jsonl \
    --config gallery/replay/benchmark/config.yaml \
    --output results/mcp-atlas \
    --warmup 5 \
    --limit 50

# 4. Stop the cluster
bash gallery/replay/benchmark/stop_cluster.sh
```
