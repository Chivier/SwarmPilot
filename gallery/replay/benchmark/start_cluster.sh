...
# Usage: bash gallery/replay/benchmark/start_cluster.sh [config.yaml]
#
...
check_port() {
    ...
    if lsof -i:"$port" > /dev/null 2>&1; then
        ...
        echo "Run: bash gallery/replay/benchmark/stop_cluster.sh"
        return 1
    fi
    return 0
}
...
echo "  Logs:             $LOG_DIR/"
echo ""
echo -e "${YELLOW}Next:${NC} bash gallery/replay/benchmark/deploy_models.sh"
