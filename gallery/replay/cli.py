...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        prog="replay",
        description="Replay LLM agent interactions for latency benchmarking",
    )
    ...
    prep = subparsers.add_parser(
        "prepare",
        help="Reshape a HuggingFace dataset into prepared replay data",
    )
    prep.add_argument(
        "--dataset",
        required=True,
        choices=["mcp-atlas", "dataclaw", "swebench-pro"],
        help="Dataset to reshape",
    )
    prep.add_argument(
        "--limit",
        type=int,
    ...
    prep.add_argument(
        "--output",
        required=True,
        help="Output JSON file path for prepared data",
    )
    ...
    run_parser.add_argument(
        "--data",
        required=True,
        help="Path to prepared data JSON (output of 'prepare')",
    )
    run_parser.add_argument(
        "--config",
        required=True,
