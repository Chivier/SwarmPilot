...
# Usage: bash gallery/mcp_atlas/scripts/start_cluster.sh [config.yaml]
#
...
check_port() {
    ...
    if lsof -i:"$port" > /dev/null 2>&1; then
        ...
        echo "Run: bash gallery/mcp_atlas/scripts/stop_cluster.sh"
        return 1
    fi
    return 0
}
...
echo "  Logs:             $LOG_DIR/"
echo ""
echo -e "${YELLOW}Next:${NC} cd gallery/mcp_atlas && python -m replay.cli run ..."
