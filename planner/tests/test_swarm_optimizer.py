...
class TestObjectiveFunction:
    ...
    def test_invalid_deployment_returns_inf(self, optimizer):
        ...
        score = optimizer.objective_function(invalid_deployment, method='relative_error')
...
class TestValidation:
    ...
    def test_invalid_B_shape(self):
        ...
        with pytest.raises(AssertionError, match="Batch capacity matrix dimension error"):
            SimulatedAnnealingOptimizer(
                M=2, N=2, B=B, initial=initial, a=0.5, target=target
            )
    ...
    def test_invalid_target_length(self):
        ...
        with pytest.raises(AssertionError, match="Target distribution vector length error"):
            SimulatedAnnealingOptimizer(
                M=2, N=2, B=B, initial=initial, a=0.5, target=target
            )
    ...
    def test_invalid_initial_model_id(self):
        ...
        with pytest.raises(AssertionError, match="Initial state contains invalid model ID"):
            SimulatedAnnealingOptimizer(
                M=2, N=2, B=B, initial=initial, a=0.5, target=target
            )
    ...
    def test_zero_capacity_deployment(self):
        ...
        with pytest.raises(AssertionError, match="Initial state contains invalid deployment"):
            SimulatedAnnealingOptimizer(
                M=2, N=2, B=B, initial=initial, a=0.5, target=target
            )
...
class TestCompareAlgorithms:
    ...
    def test_compare_algorithms_basic(self):
        ...
        # Should have simulated annealing results
        assert 'simulated_annealing' in results
        assert 'deployment' in results['simulated_annealing'] or 'error' in results['simulated_annealing']
    ...
    def test_compare_algorithms_with_ip(self):
        ...
        # Should have both algorithm results when pulp is available
        assert 'simulated_annealing' in results
        assert 'integer_programming' in results
