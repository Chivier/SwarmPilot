...
Endpoints:
    ...
    POST /pylet/scale  - Scale a specific model
    POST /pylet/migrate - Migrate an instance
    POST /pylet/optimize - Run optimizer and deploy via PyLet
    POST /pylet/terminate-all - Terminate all PyLet instances
"""
...
# Create router for PyLet endpoints
router = APIRouter(prefix="/pylet", tags=["pylet"])
...
@router.post("/deploy", response_model=PyLetDeploymentOutput)
async def pylet_deploy(input_data: PyLetDeploymentInput):
    ...
    Args:
        input_data: Target state configuration.
    ...
    logger.info(f"PyLet deploy request: target_state={input_data.target_state}")
    ...
    try:
        ...
        logger.error(f"PyLet deploy failed: {e}")
        raise HTTPException(
