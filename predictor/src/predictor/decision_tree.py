...
Provides expected runtime and error margin predictions using Scikit-Learn's
...
class DecisionTreePredictor(BasePredictor):
    """Predictor using Decision Tree Regression.

    Trains a Decision Tree model to predict expected runtime and
    ...
    def predict(self, features: dict[str, Any]) -> dict[str, Any]:
        ...
        if self.model is None:
            ...
            logger.error(
                f"DecisionTreePredictor prediction failed: {error_msg}"
            )
            raise ValueError(error_msg)
    ...
    def get_model_state(self) -> dict[str, Any]:
        ...
        if self.model is None:
            ...
            logger.error(
                f"DecisionTreePredictor get_model_state failed: {error_msg}"
            )
            raise ValueError(error_msg)
